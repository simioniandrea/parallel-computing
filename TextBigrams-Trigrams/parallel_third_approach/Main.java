...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // /Users/andreasimioni/Desktop/parallelComputing/mid_term_BIGRAMS_TRIGRAMS/TextBigrams-Trigrams/Java/sample
        ...
        try (PrintWriter out = new PrintWriter("result.txt")){
            for (int p = 0; p < size; p++) {
                ...
                for (int j = 0; j < numberOfRuns; j++) {
                    ...
                    ArrayList<Future> f_array = new ArrayList<>();  // Create an ArrayList of Future that will contain the threads results
                    ...
                    for (int i = 0; i < n_thread; i++) {
                        Future f = exec.submit(new Parallel_thread("t" + i, i * k, ((i + 1) * k) + (n_g - 1) - 1, n_g, text)); //executor.submit start the Callable call that save the results in a Future
                        f_array.add(f);
                    }
                    try {
                        for (Future<HashMap<String, Integer>> f : f_array) {
                            HashMap<String, Integer> n_grams = f.get(); // f.get return the thread result
                            HashMerge.merge(n_grams, finalNgrams); //merge in finalNgrams
                        }
